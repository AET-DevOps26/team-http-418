	...
	private class ProfileRowMapper implements RowMapper<Profile> {
		...
		public Profile mapRow(ResultSet rs, int rowNum) throws SQLException {
			try {
				...
				final Profile.Student student = objectMapper.readValue(
						rs.getString("student"),
						Profile.Student.class);

				// Deserialize the Lists using TypeReferences
				final List<String> completed = objectMapper.readValue(
						rs.getString("completed_courses"),
						new TypeReference<List<String>>() {
						});

				final List<String> enrolled = objectMapper.readValue(
						rs.getString("enrolled_courses"),
						new TypeReference<List<String>>() {
						});

				final List<String> available = objectMapper.readValue(
