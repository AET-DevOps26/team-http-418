...
class DB:
    ...
    async def create_instance(cls, debug: bool = False):
        ...
        await instance.create_connection_pool() #must be after setup
        await instance.init_tables()
        return instance

    # noinspection PyUnresolvedReferences
    async def create_connection_pool(self):
        self.connection_pool = await asyncpg.create_pool( # is marked as not async but it is
            user=DB_USER,
            password=DB_PASS,
            database=DB_NAME,
            host=DB_HOST
...
async def update_database(pool: Pool, data: tuple[ET.Element, ET.Element]) -> None:
